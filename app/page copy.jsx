...
// Komponen IP Card
// Komponen IP Card dengan error details
const IPCard = ({ ip, data }) => {
  ...
  const handleClick = (e) => {
    ...
    // Atau gunakan approach yang lebih aman:
    const newWindow = window.open("", "_blank");
    if (newWindow) {
      newWindow.opener = null;
      newWindow.location.href = `http://${ip}`;
    }
  };
...
// Komponen Compact Point View dengan error
...
export default function Home() {
  ...
  // Di bagian atas komponen utama, tambahkan state untuk search:
  const [searchQuery, setSearchQuery] = useState("");
  const [searchFilters, setSearchFilters] = useState({
    type: "all", // all, public, vpn
    status: "all", // all, online, warning, offline
    sortBy: "name", // name, responseTime, uptime
  });

  // Gunakan WebSocket hook
  const { isConnected, requestUpdate } = useWebSocketMonitor();
  ...
  // Update UI untuk tunjukkan connection status
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      {/* Header dengan connection indicator */}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="p-2 bg-slate-700 hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg transition-all flex items-center gap-1"
              >
                <RefreshCw
                ...
                />
                <span className="text-xs text-slate-300">Refresh</span>
              </button>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-red-300">
                  Real-time monitoring is unavailable. Attempting to
                  reconnect...
                </div>
              </div>
            </div>
  ...
  // Di dalam komponen utama, tambahkan state baru
  ...
  // Simulasi check IP status (lebih realistik)
  // Fungsi check IP status dengan HTTP request sebenarnya
  const checkIPStatus = async (ip) => {
    const start = Date.now();

    return new Promise((resolve) => {
      const img = new Image();
      const url = `${ip.replace(/\/$/, "")}/favicon.ico?ping=${Date.now()}`;
      ...
      const done = (status, error = null) => {
        ...
        resolve({
          ...
          uptime: status === "online" ? 99 : 0,
          lastChecked: new Date(),
          accessible: status === "online",
          error: status === "online" ? null : error,
          errorDetails: error,
        });
      };

      // Jika load berhasil → IP online
      img.onload = () => done("online");

      // Jika gagal → IP offline / port mati
      img.onerror = () => done("offline", "Host unreachable / Port closed");
  ...
  //       status: "online",
  //       responseTime,
  //       uptime: Math.floor(Math.random() * 30) + 70, // Simulasi uptime
  //       lastChecked: new Date(),
  //       accessible: true,
  //       error: null,
  //       errorDetails: null,
  ...
  //       errorDetails = "Server took too long to respond (over 5 seconds)";
  //     } else if (error.message.includes("Failed to fetch")) {
  //       errorType = "Network Error";
  //       errorDetails = "Failed to establish connection to server";
  ...
  // Update status semua IP - optimasi untuk 40 IP
  const updateAllStatuses = useCallback(async () => {
    ...
    try {
      // Gabungkan semua IP untuk parallel checking
      const allIPs = [...IP_CONFIG.public, ...IP_CONFIG.vpn];

      // Gunakan Promise.all dengan batasan concurrent untuk menghindari overload
      const batchSize = 10;
      const batches = [];
      ...
      // Process batches sequentially but items in batch concurrently
  ...
  // Auto refresh handler yang terpisah
  useEffect(() => {
    let intervalId = null;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="relative w-40 h-10 md:w-50 md:h-12  rounded-lg flex items-center justify-center overflow-hidden">
                {/* Mengganti tag <img> lama dengan komponen Next.js Image */}
                <img
                  src="/17.png" // Path di Next.js dimulai dari folder ROOT publik (app)
                  alt="Dzyra Technology Logo"
                  width={560} // Sesuaikan lebar dan tinggi untuk ukuran yang diinginkan
                  height={280}
                  className="w-40 h-60 md:w-70 md:h-90 object-contain relative z-10"
                  // priority // Tambahkan priority karena ini adalah elemen LCP
                />
                {/* Efek latar belakang untuk logo */}
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        {/* 🔥 Manual IP Testing Panel - TARUH DI SINI 🔥 */}
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4 mb-6">
          ...
          <div className="flex gap-2 mb-3">
            <input
              ...
              value={testingIP}
              onChange={(e) => setTestingIP(e.target.value)}
              placeholder="Enter IP:Port (e.g., 192.168.1.1:80)"
              className="flex-1 px-3 py-2 bg-slate-900/50 border border-slate-700 rounded text-white text-sm"
            />
            <button
              onClick={() => testSingleIP(testingIP)}
              className="px-4 py-2 bg-cyan-500 hover:bg-cyan-600 text-white rounded text-sm font-semibold"
            >
              Test
            </button>
          </div>
